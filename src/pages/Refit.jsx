    ...
    <div className="refit-page">
      ...
      <section className="relative py-32 bg-gradient-to-br from-navy-900 to-ocean-600 text-white overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 text-center">
          <h1 className="text-5xl md:text-6xl font-bold mb-6">{t('refit.hero.title')}</h1>
          <p className="text-xl md:text-2xl text-gray-200 max-w-3xl mx-auto mb-8">{t('refit.hero.subtitle')}</p>
          <Link 
            to="/contato"
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-navy-900 mb-4">{t('refit.services.sectionTitle')}</h2>
            <p className="text-xl text-gray-600">{t('refit.services.sectionSubtitle')}</p>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-navy-900 mb-4">{t('refit.process.title')}</h2>
            <p className="text-xl text-gray-600">{t('refit.process.subtitle')}</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {refitProcess.map((phase, index) => (
              <div key={index} className="relative bg-gradient-to-br from-ocean-500 to-ocean-600 p-8 rounded-xl text-white">
                <div className="text-6xl font-bold opacity-20 absolute top-4 right-4">{phase.step}</div>
      ...
      <section className="py-20 bg-navy-900 text-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-4xl md:text-5xl font-bold mb-6">{t('refit.why.title')}</h2>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {Array.isArray(whyRefit) && whyRefit.map((reason, index) => (
              <div key={index} className="bg-navy-800 p-6 rounded-lg flex items-center gap-4">
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-navy-900 mb-4">{t('refit.examples.title')}</h2>
          </div>

          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {examples.map((example, idx) => (
              <div key={idx} className="bg-white rounded-xl overflow-hidden shadow-lg">
                <img src={example.img} alt={example.title} className="w-full h-64 object-cover" />
                <div className="p-6">
                  <h3 className="text-xl font-bold text-navy-900 mb-2">{example.title}</h3>
                  <p className="text-gray-600 mb-4">{example.desc}</p>
                  <div className="text-ocean-500 font-semibold">{t('refit.examples.duration')}: {example.duration} {t('refit.examples.months')}</div>
                </div>
              </div>
            ))}
          </div>
